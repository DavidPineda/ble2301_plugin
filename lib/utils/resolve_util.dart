...
class ResolveUtil {
  ...
  static Map getSleepData(List<int> value) {
    ...
    //   var stringBuffer = StringBuffer();
    //   for (int j = 0; j < sleepLength; j++) {
    //     stringBuffer.write(_hexByte2Int(value[10 + j + i * 34], 0).toString());
    //     stringBuffer.write(j == sleepLength ? "" : " ");
    //   }
    //   hashMap.addAll({DeviceKey.ArraySleep:stringBuffer.toString()});
    //   list.add(hashMap);
    // }
    // return maps;
  }
  ...
  static Map measurementWithType(bool flag, List<int> value) {
    if (flag) {
      switch (value[1]) {
        case 1: //hrv
          return setMethodSuccessful(BleConst.MeasurementHrvCallback);
        case 2: //heart
          return setMethodSuccessful(BleConst.MeasurementHeartCallback);
        case 3: //0xy
          return setMethodSuccessful(BleConst.MeasurementOxygenCallback);
      }
    } else {
      switch (value[1]) {
        case 1: //hrv
